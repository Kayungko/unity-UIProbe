...
- 🛠️ **实用工具集** - 图片规范化、大红大金资源导入、高清截图、TMP 富文本生成器等
- ⚡ **预制体助手** - 快速创建 UI 节点、九宫格锚点、智能布局应用

本文档详细介绍了 v3.11.0 版本的所有功能模块。
...
2. [功能模块详情](#2-功能模块详情)
    ...
    - [2.6 实用工具](#26-实用工具)
        ...
        - [大红大金资源修改导入 (Red/Gold Resource Importer)](#大红大金资源修改导入-redgold-resource-importer)
        - [游戏截屏 (Screenshot)](#游戏截屏-screenshot)
        - [TMP 富文本生成器](#tmp-富文本生成器)
    - [2.7 预制体助手 (Prefab Helper)](#27-预制体助手-prefab-helper)
    - [2.8 关于页与一键更新 (About & Auto Update)](#28-关于页与一键更新-about--auto-update)
3. [设置与配置](#3-设置与配置)
...
2.  **Raycast Target 优化**: 扫描开启了 `Raycast Target` 但没有添加交互组件（如 Button）的图片/文本，这些组件会无谓地消耗由于射线检测带来的性能。
    ...
    *   *CSV 导出*: 在“不必要的 Raycast Target”结果分组中点击 **"导出CSV"**，可导出当前预制体的射线检测清单，字段包含预制体路径、节点路径、组件类型、Raycast Target 状态和问题描述。
...
*   **索引刷新同步**: 在「预制体索引」页刷新工程预制体后，资源引用页会自动使用最新索引重新计算结果，避免继续显示旧节点路径。
*   **完整节点显示**: 引用位置中的节点路径会显示完整层级，并支持长名称自动换行；Image/RawImage 引用说明以黄色显示，便于快速区分图片类引用。
*   **点击定位节点**: 蓝色节点路径可点击，点击后会打开对应预制体，并尝试在 Prefab Stage/层级结构中选中该节点。
*   **CSV 导出**: 搜索到结果后，点击结果标题右侧的 **"导出CSV"**，可导出当前搜索结果。表格包含搜索资源类型、搜索关键字、引用类型、预制体名称、预制体路径、节点路径、资源名称、资源路径和额外信息。
...
*   **Device Simulator 支持**: 兼容 Unity Device Simulator 的触摸模拟模式，并会遵守当前选择的拾取方式，避免普通点击绕过设置误触发拾取。
...
#### 大红大金资源修改导入 (Red/Gold Resource Importer)
*   **功能**: 位于”图片工具”下的子标签，用于读取 CSV/TSV 表格，根据名称或图标路径匹配源图片，并按品质输出到红、紫、金资源目录。
*   **表格字段**:
    *   默认识别 `名称`、`品质`、`格数：长`、`格数：宽`、`格数`、`图标路径`，也可以在面板中手动改列名。
    *   生成后会把新的图标路径写回 `图标路径` 列；启用统一格数时也会同步回写格数列。
*   **输出规则**:
    *   红 / 紫 / 金品质分别配置输出路径，工具会根据品质字段自动分流。
    *   1:1 到 6:6 的正方形资源默认统一输出为 512x512。
    *   其它比例按”格数 × 单格像素”计算，例如 2:3 在默认 100px 单格下输出 200x300。
*   **预览与安全性**:
    *   预览结果按”可替换图片资源 / 未找到匹配资源 / 源目录未匹配资源”分组展示。
    *   可替换资源会标记新增、修改、无变化状态；同名源图冲突时自动优先使用最后修改时间最新的文件。
    *   可在预览中切换保留已有输出或改用源图，也可以直接编辑最终输出文件名。
    *   覆盖已有输出前会自动备份旧文件，生成后可通过面板撤销按钮恢复旧图片和表格字段。
    *   **栈式多级撤销 (v3.5.0)**: 撤销系统支持最多 10 次操作回退，按钮显示当前可撤销次数 `↩ 撤销 (N)`，超限自动清理最旧备份。
*   **操作流程**:
    1.  选择表格文件和待处理图片文件夹。
    2.  配置列名、输出目录、尺寸规则和表格回写方式。
    3.  点击”读取表格并预览”，检查问题项、未匹配源图、变更状态和输出路径。
    4.  确认后点击”生成资源并写回表格”。

#### 游戏截屏 (Screenshot)
*   **功能**: 在 Play 模式或 Scene/Prefab 视图下截取高质量画面，所见即所得。
*   **特性**:
    *   **双模式支持**:
        *   **Game View**: 运行时截图，分辨率**自动同步**到 Game 窗口，彻底解决宽高比拉伸问题。
        *   **Scene/Prefab**: 直接对当前编辑的预制体截图，无需运行游戏。
    *   **自动对焦 (Auto Frame)**: (Scene/Prefab模式) 智能识别 **Canvas (Environment)** 或 Root 节点，自动适配视角，完美匹配设计分辨率。
    *   **超采样**: 支持 1x-4x 超大分辨率 (如 4K/8K 级截图)。
    *   **透明背景**: 支持去背截图（需相机设置 Clear Flags 为 Solid Color + Alpha 0）。
    *   **仅截 UI 层 (新)**: 支持双背景差值合成（两帧渲染，黑底+白底），专门解决各种 Shader 及特效写入 Alpha 问题，导出完美 PNG。也可选择单帧透明渲染模式以追求极致速度。
    *   **快捷键**: `F12` (Play模式)。
...
### 2.8 关于页与一键更新 (About & Auto Update)

在侧边栏底部的 **"关于"** 页面，可查看版本号、开发者信息、核心功能列表与历史更新日志，并支持 **编辑器内一键升级** (v3.11.0 新增)。

*   **检查更新**:
    *   工具每 24 小时自动后台检测一次最新版本（基于 GitHub Release），不打扰正常工作流。
    *   也可在关于页点击 **"🔄 手动检查更新"** 立即检测。
*   **一键自动更新** ✨ *v3.11.0 新功能*:
    *   检测到新版本后，弹出三选一对话框：
        *   **立即更新 (自动导入)**: 直接在编辑器内下载最新 `.unitypackage`，带 **可取消的进度条**，下载完成后自动触发 Unity 导入窗口，确认即完成升级。
        *   **稍后再说**: 关闭提示，下次再处理。
        *   **前往下载页**: 在浏览器打开 GitHub Release 页面手动下载。
    *   **安全回退**: 若自动下载/导入失败，会自动回退到浏览器下载页，不会卡住。
    *   **零依赖**: 全程不依赖 git / CLI，也不要求工程绑定 GitHub 仓库。
*   **资源链接**: 关于页同时提供 **"📖 查看 README"** 和 **"🌐 GitHub 仓库"** 快捷入口。
...
*   **设置项**:
    ...
    *   **功能模块显示设置**: 可隐藏/显示主要功能入口，并可单独控制 `Filter节点排查` 与 `大红大金资源修改导入` 子功能入口。
    *   **拾取习惯**: 修改 Runtime Picker 的触发热键。
    *   **自动更新**: 开启/关闭检测到新预制体时的自动刷新。

---
*文档生成于 UIProbe v3.11.0*
