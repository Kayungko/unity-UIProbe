...
**UIProbe** 是一个强大的 Unity UI 开发辅助工具集，旨在解决 UI 开发中的痛点，如重命名引用丢失、性能优化、界面调试和资源管理。
...
2. [功能模块详情](#2-功能模块详情)
    ...
    - [2.4 运行时拾取 (Runtime Picker)](#24-运行时拾取-runtime-picker)
    - [2.5 界面记录与回放 (Recorder & Browser)](#25-界面记录与回放-recorder--browser)
    - [2.6 实用工具](#26-实用工具)
        - [图片规范化 (Image Normalizer)](#图片规范化-image-normalizer)
        - [游戏截屏 (Screenshot)](#游戏截屏-screenshot)
        - [TMP 富文本生成器](#tmp-富文本生成器)
    - [2.7 适配助手 (UI Adaptor)](#27-适配助手-ui-adaptor)
3. [设置与配置](#3-设置与配置)
...
### 2.5 界面记录与回放 (Recorder & Browser)

用于调试复杂的 UI 状态变化。

*   **UI Recorder (录制器)**:
    *   选择一个 `UI Root` (如 Canvas)。
    *   点击 **"开始记录"**。
    *   此时游戏中的 UI 显隐、层级变化会被录制下来。
    *   支持手动添加 **"快照"** (Snapshot) 和 **"标签"** (Tag)。
*   **History Browser (浏览器)**:
    *   回放之前的录制会话。
    *   **Diff 视图**: 对比两个时间点或两个会话之间的 UI 结构差异（新增/删除/移动的节点会高亮显示）。
