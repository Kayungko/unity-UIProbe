...
### 🏗️ 大红大金模块架构重构
- **模块化拆分**: 原 1794 行巨型文件拆分为 1 个 UI 层 + 7 个 Data 层独立类，职责清晰、易于维护。
- **栈式多级撤销**: 撤销系统从单级升级为栈式多级（最多 10 层），按钮显示 `↩ 撤销 (N)` 和剩余可撤销次数，超限自动清理最旧备份。
- **逻辑层独立**: CSV 解析器（`DelimitedFileParser`）、拼音命名转换（`RedGoldNameConverter`）、图片匹配器（`RedGoldImageMatcher`）等均可被其他模块复用。
- **开发文档同步更新**: `UIProbe_ImageTools_DevDoc.md` 第 2/5/6 节全面重写，反映新架构。
