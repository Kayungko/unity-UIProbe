...
实现和维护 `ui-check` 模块。在声明的写入路径内工作，遵守模块边界。
...
> **导航提示**: 优先通过 `.claude/wiki/graph.json`（god_nodes / communities）定位 `ui-check` 相关节点后，按需局部读取 wiki 页面。当 graph.json 不存在时，直接读取下方 wiki 链接。
- [Wiki: ui-check](.claude/wiki/modules/ui-check.md) — 本模块领域文档（graph 不可用时直读）
- [TDD Spec](.claude/tdd/specs/ui-check.spec.md) — 测试规格
