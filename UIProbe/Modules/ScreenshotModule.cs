...
namespace UIProbe
{
    internal sealed class ScreenshotModule : UIProbeModuleBase
    {
        ...
        /// <summary>
        /// 截屏页签快捷键检测（仅当本页签激活且 Play 模式时由壳层 Update 分发）
        /// </summary>
        public override void OnWindowUpdate()
        {
        ...
        private void OnUICameraPostRender(ScriptableRenderContext ctx, Camera cam)
        {
            ...
            try
            {
                if (_uiCapturePhase == 3)
                {
                    ...
                    UnityEngine.Object.DestroyImmediate(_uiCaptureRT); _uiCaptureRT = null;

                    byte[] singleBytes = singleShot.EncodeToPNG();
                    File.WriteAllBytes(_uiCapturePath, singleBytes);
                    UnityEngine.Object.DestroyImmediate(singleShot);
                ...
                else if (_uiCapturePhase == 2)
                {
                    ...
                    UnityEngine.Object.DestroyImmediate(_uiCaptureRT);
                    _uiCaptureRT = null;

                    // 差值合成
                    Texture2D result = CompositeFromBlackAndWhite(_uiCaptureBlackTex, whiteTex);
                    UnityEngine.Object.DestroyImmediate(_uiCaptureBlackTex); _uiCaptureBlackTex = null;
            ...
            catch (Exception ex)
            {
                ...
                if (_uiCaptureWasOverlay && _uiCaptureUrpData != null && _uiCaptureRenderProp != null)
                    _uiCaptureRenderProp.SetValue(_uiCaptureUrpData, 1);
                if (_uiCaptureRT != null)       { UnityEngine.Object.DestroyImmediate(_uiCaptureRT);       _uiCaptureRT       = null; }
                if (_uiCaptureBlackTex != null) { UnityEngine.Object.DestroyImmediate(_uiCaptureBlackTex); _uiCaptureBlackTex = null; }
                _uiCaptureInProgress = false;
                _uiCapturePhase      = 0;
